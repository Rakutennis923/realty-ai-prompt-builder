  ...
  const prompt = useMemo(() => {
    ...
    const imageSpec =
      mode === "image"
        ...
        : "";
    return `${role}\n\n請依照以下資料製作「${title}」。\n\n【任務要求】\n${direction}${imageSpec}\n\n【物件資料】\n${property}\n\n【聯絡及經紀業資料】\n${contact || "（尚未填寫）"}\n\n【台灣房仲廣告防呆規則】\n1. 只能使用上述資料，不得自行虛構價格、坪數、用途、學區、交通時間、公共建設、屋況、景觀、投報率、成交紀錄或買方人數。\n2. 資訊不足時標示「待確認」，不可用推測補足。\n3. 不使用「保證增值、絕對最低價、唯一釋出、保證入學、穩賺」等無法證明的說法。\n4. 未完成的交通或公共建設須明確標示目前狀態，不得呈現為已完成。\n5. 先檢查內容與資料是否一致，再輸出完整成品。`;
