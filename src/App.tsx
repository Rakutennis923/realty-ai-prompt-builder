...
const portraitDirections: Record<string, string> = {
  ...
  "younger-pro":
    "保留本人真實年齡與辨識度，只自然減輕疲態、黑眼圈與細小紋理，使整體看起來精神、專業並自然年輕約3至5歲；穿著西裝外套時，在左胸別上小巧精緻的金色金屬『太平洋房屋』徽章，字樣使用正確繁體中文，不得變形或改字。禁止童顏化、換臉或改變臉型。",
  "natural-retouch":
...
const portraitSkinDirections: Record<string, string> = {
  ...
  高質感美肌:
    "先依本人原始臉況判斷後，進行高質感雜誌級美肌與極輕微輪廓平衡：臉部過圓或過寬時只自然收整約3%至5%，臉部過瘦或凹陷時適度增加健康飽滿感，並輕微改善左右不對稱、浮腫與下顎線條，使男性更帥氣俐落、女性更漂亮有氣質。同步讓膚色明亮均勻、膚質細緻、眼神清晰、輪廓光乾淨。只能微調軟組織與視覺比例，不得改變骨相、原始臉型特色、五官比例或本人辨識度，也不得製造失真的陶瓷肌、尖下巴或網紅臉。",
};
