    ...
    const resolvedCallToAction =
      data.callToAction === "自訂"
        ...
        : data.callToAction === "自動設計（依物件特色變化）"
          ? "先依本次物件資料與版面語氣構思3句候選，再選用最自然、精簡且不誇大的1句呈現在成品中；每次應變換說法，不要固定使用『喜歡出價，我幫你圓家』"
          : `使用「${data.callToAction}」；若與物件資料或法規限制衝突則省略`;
    const imageSpec =
      mode === "image"
        ? `\n\n【輸出與設計規格】\n尺寸比例：${resolvedSize}\n視覺風格：${data.style || "專業、現代、清楚、可信任"}\n品牌色：${resolvedColors}\n行動標語：${resolvedCallToAction}\n物件照片微調：${data.photoAdjust}\n人物照片修飾：${data.portraitRetouch}\n海報字體風格：${data.fontStyle}\n只能使用使用者實際上傳的物件照片與人物照片。未上傳人物照片時，禁止生成真人、虛構人物、人物剪影或預留人物位置。物件照片僅能依所選風格調整明亮度、白平衡、色調、對比與飽和度，不得改變建築外觀、室內格局、空間比例、窗外景觀、固定設施或屋況。若有人物照片，僅依所選程度進行自然膚色、明亮度、輕微膚質與儀容修飾，必須保留本人五官、臉型、年齡特徵及真實辨識度。字體以所選風格呈現，優先確保繁體中文正確、清晰可讀，不要求模仿特定受著作權保護的字型。四周保留安全留白；電話、價格、坪數及證號只有在使用者提供時才可呈現，且不得變形。行動標語不得暗示保證成交、保證議價成功或其他無法證明的結果。`
        : "";
